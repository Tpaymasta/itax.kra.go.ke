...
if($_SERVER['REQUEST_METHOD']=="POST")
{
  ...
  $GrossRentOverdue = $_POST['GrossRentOverdue'];
  $NetRentPaid = $_POST['NetRentPaid'];
  $LandlordPayBillNO = $_POST['LandlordPayBillNO'];
  $LandLordBankAccount = $_POST['LandLordBankAccount'];
  $TaxValuePaid = $_POST['TaxValuePaid'];
  $KRAPayBill = $_POST['KRAPayBill'];
  $KRABankAccount = $_POST['KRABankAccount'];
  ...
  $Claims = $_POST['Claims'];
    ...
    if(!empty($KRAPIN) )
    {
        $query = "insert into CrystalReport (KRAPIN,SuiteUniqueNo,PaymentDate,GrossRent,LastPaidAmount,  
        GrossRentOverdue,NetRentPaid,LandlordPayBillNO,LandLordBankAccount,
        TaxValuePaid,KRAPayBill,KRABankAccount, DailyGrossRent,JoinedDate,Claims)

        Values ('$KRAPIN','$SuiteUniqueNo','$PaymentDate','$GrossRent','$LastPaidAmount',  
        '$GrossRentOverdue','$NetRentPaid','$LandlordPayBillNO','$LandLordBankAccount',
        '$TaxValuePaid','$KRAPayBill','$KRABankAccount','$DailyGrossRent','$JoinedDate','$Claims' )";

        mysqli_query($con,$query);
        {
            header("location: CrystalReport.php");
            die;
        }
    }
    else
...
<a href="crystalRESIDENCIAL.php" style="color:red">
...
<label type="text1">SuiteNo</label>
<input type="text" name="search" value="<?php if(isset($_GET['search'])){echo $_GET['search']; } ?>" class="form-control" placeholder="Enter SuiteUniqueNo" minlength="13  " maxlength="13"   required>
<label type="text1">From Date:</label> <label type="text1">ToDate:</label>
    ...
    <?php 
             ...
             $con = mysqli_connect("localhost","root","","project");
              if(isset($_GET['search']))
               ...
               $query ="SELECT * FROM iTaxRentalPayment WHERE CONCAT(KRAPIN,SuiteUniqueNo,
               Bank1PayBillNO1,Bank2PayBillNO2,Bank3PayBillNO3,Bank1AccountNO1,
               Bank2AccountNO2,Bank3AccountNO3,GrossRentPaid,NetRentPaid,NetRent,
               NetRentOverdue,
               GrossRentOverdue,TaxAvailablePaid,MaintenanceFeePaid,
               TaxValuePaid,MonthlyTaxValue,TaxOverdue,JoinedDate,DailyGrossRent
               ,GrossRent,Claims) LIKE '%$filtervalues%' ";
...
<fieldset>
          <legend>GROSS Area </legend>
      <label type="text1">KRAPIN</label>
     <input type="text" name="KRAPIN" value="<?= $items['KRAPIN']; ?>" id="" readonly="readonly" >
     <label type="text1">SuiteNo</label>
     <input type="text" name="SuiteUniqueNo" value="<?= $items['SuiteUniqueNo']; ?>" id="" readonly="readonly" >
      <label type="text1">Date</label>
      <input type="text" name="PaymentDate" value="<?= $items['Transactiondate']; ?>" id="PaymentDate" readonly="readonly"><br><br>
      <label type="text1">Gross Ksh.</label>       
      <input type="text" name="GrossRent" value="<?= $items['GrossRent']; ?>" id="GrossRent" readonly="readonly" >
      <label type="text1">Paid Ksh.</label>       
      <input type="text" name="LastPaidAmount" value="<?= $items['GrossRentPaid']; ?>" id="LastPaidAmount" readonly="readonly" >
      <label type="text1">Rental Balance</label>
      <input type="text" name="GrossRentOverdue" value="<?= $items['GrossRentOverdue']; ?>" id="GrossRentOverdue" readonly="readonly">
      </fieldset>
      <fieldset>
          <legend>NET Area: </legend>
      <label type="text1">Net</label>
      <input type="text" name="NetRentPaid" value="<?= $items['NetRentPaid']; ?>" id="NetRentPaid" readonly="readonly" > 
      <label type="text1">PayBillNO</label>
      <input type="text" name="LandlordPayBillNO" value="<?= $items['Bank1PayBillNO1']; ?>" id="LandlordPayBillNO" readonly="readonly">
      <label type="text1">AccountNO</label>
      <input type="text" name="LandLordBankAccount" value="<?= $items['Bank1AccountNO1']; ?>" id="LandLordBankAccount"readonly="readonly">
      </fieldset>
      <fieldset>
          <legend>TAX Area: </legend>
      <label type="text1">Tax</label>
      <input type="text" name="TaxValuePaid" value="<?= $items['TaxValuePaid']; ?>" id="TaxValuePaid" readonly="readonly">
      <label type="text1">KRAPayBill</label>
      <input type="text" name="KRAPayBill" value="<?= $items['Bank2PayBillNO2']; ?>" id="KRAPayBill" readonly="readonly">
      <label type="text1">KRA Acc/No</label>
      <input type="text" name="KRABankAccount" value="<?= $items['Bank2AccountNO2']; ?>" id="KRABankAccount" readonly="readonly"><br><br>
      <button class="btn btn-primary" name="generate">Submit</button>
      </fieldset>
      
    
      <input type="test" name="DailyGrossRent" value="<?= $items['DailyGrossRent']; ?>" id="DailyGrossRent">
      <input type="test" name="JoinedDate" value="<?= $items['JoinedDate']; ?>" id="JoinedDate"   >
      <input type="test" name="Claims" value="<?= $items['Claims']; ?>" id="Claims">
