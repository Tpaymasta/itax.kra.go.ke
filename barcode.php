...
class="card-img-top mx-auto" style="width:100%">
...
<?php
    ...
    <tr>
      ...
      <td colspan="3">
     <strong> Barcode NO:<?php echo $code2 ?>
...
<table>
  <thead>
    <tr>
      ...
      <td colspan="2"><strong>Payment ReceiptNo:</strong>
      <?php
					include('conn1.php');
					$query=mysqli_query($conn,"select * from `iTaxsignup`");
					while($row=mysqli_fetch_array($query)){
						?>
    <input type="tel" name="KRAPIN" value="<?php echo "" . date("Ymdhms"); ?>"  readonly="readonly">
...
<?php
    ...
    <th>Sr.NO.</th>
      ...
      <th><strong>Tax Sumery (Ksh)</strong></th>
    </tr>
     ...
     <tr>
      <td colspan="4"><strong>Total Tax Paid (Ksh)</strong></td>
      <th>
      <strong>
      <?php
$results = $db->prepare("SELECT sum(TaxValuePaid) FROM itaxrentalpayment");
...
?></strong>
     ...
     <tr>
      <td colspan="4"><strong>Total Tax Expected (Ksh)</strong></td>
      <th>
      <strong>
      <?php
$results = $db->prepare("SELECT sum(MonthlyTaxValue) FROM itaxrentalpayment");
...
?></strong>
     ...
     <tr>
      <td colspan="4"><strong>Excess Tax Paid (Claims)Ksh. </strong></td>
      <th>
      <strong>
      <?php
